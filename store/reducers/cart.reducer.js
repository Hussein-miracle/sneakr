...
const cartReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    case ADD_TO_CART:
      ...
      const cartItemIndex = updatedCartItems.findIndex((cItem) => cItem.id === id);

      if(cartItemIndex >= 0){
        updatedCartItems[cartItemIndex].quantity =  updatedCartItems[cartItemIndex].quantity + 1; 
        ...
        updatedCartItemsCount = updatedCartItems.reduce((acc,item) => acc + item.quantity,0);
